...
function renderScenes(){
 ...
 $("#sceneList").innerHTML=scenePlaces.filter(p=>p.title[lang].toLowerCase().includes(q)).map((p,i)=>`<div class="sceneItem ${i===current?"active":""}" onclick="loadScene(${i})"><img class="sceneThumb" src="${p.img}"><span><b>${p.title[lang]}</b><small>360° panorama</small></span></div>`).join("");
...
function renderCards(){
 ...
 $("#placeCards").innerHTML=list.map((p)=>`<article class="placeCard"><img src="${p.img}"><div class="placeBody"><span class="tag">${catName(p.cat)}</span><h3>${p.title[lang]}</h3><p>${p.desc[lang]}</p><div class="cardActions"><button class="main" onclick="loadScene(${places.indexOf(p)});scrollToId('home')">360°</button><button onclick="showDetails('${p.id}')">${tr("details")}</button><button onclick="toggleFav('${p.id}')">♥</button></div></div></article>`).join("");
}
$("#search").oninput=renderScenes;$("#placeSearch").oninput=renderCards;$("#catFilter").onchange=renderCards;
function loadScene(i){current=(i+places.length)%places.length;if(viewer){viewer.loadScene(places[current].id,0,0,100)}renderScenes();renderThumbs();renderDetail();lucide.createIcons()}
...
const map=L.map("leafletMap").setView([40.49,68.783],14);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap",maxZoom:19}).addTo(map);
const markers=[];places.forEach((p,i)=>{markers[i]=L.marker([p.lat,p.lng]).addTo(map).bindPopup(`<b>${p.title.uz}</b><br><button onclick="loadScene(${i});scrollToId('home')">360°</button>`)});
function fitMap(){map.fitBounds(L.latLngBounds(places.map(p=>[p.lat,p.lng])).pad(.15))}
function initViewer(){
